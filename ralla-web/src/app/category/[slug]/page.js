...
export default async function CategoryPage({ params }) {
  ...
  // Anime Filters
  const donghuaList = allMovies.filter(m => m.genre && m.genre.toLowerCase().includes('donghua'));
  const animeList = allMovies.filter(m => m.genre && m.genre.toLowerCase().includes('anime') && !m.genre.toLowerCase().includes('donghua'));
  // Other Animation Grid එකට
  const otherAnimation = allMovies.filter(m => m.genre && !m.genre.toLowerCase().includes('donghua') && !m.genre.toLowerCase().includes('anime'));
